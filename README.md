...
# 実行方法
1. Unicode.orgから 絵文字の定義ファイル(emoji-data.txt)をダウンロードし、任意のディレクトリに格納する
  http://unicode.org/Public/emoji/6.0/emoji-data.txt
  
### (Xcodeで実行の場合)
2. XcodeからEmojiExtractor.xcodeprojを開く
3. EditSchemeを選択する
...
2. コマンドラインからswiftファイルをコンパイルし、実行する
```
$ swiftc emoji.swift main.swift
$ ./main /xxx/yyy/emoji-data.txt
```
